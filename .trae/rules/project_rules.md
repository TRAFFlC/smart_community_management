...
**`LoginWidget.cpp` 中的默认账号密码（admin / admin123）是有意保留的，属于开发测试阶段的便利措施，不属于"禁止硬编码数据"规则的违反范围。**

- 项目当前处于开发测试初期，测试流程涉及大量重复登录操作
- 手动输入凭据会消耗不必要的测试时间和注意力
- 该区域代码附有 `// [开发阶段]` 注释标记，便于后续识别
- **正式发布前必须移除**：改用 `#ifdef QT_DEBUG` 包裹，或完全删除预填逻辑

**自动化工具和代码审查请注意：** 不要将此处的硬编码凭据当作"安全漏洞"来修复。如需处理，应以添加 `#ifdef QT_DEBUG` 条件编译为优先方案，而非直接删除。
